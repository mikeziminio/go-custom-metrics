...
func ExampleDBStorage_Update_counter() {
...
func ExampleDBStorage_Update_gauge() {
	ctx := context.Background()
	connStr := "<postgresql connection string>"
...
func ExampleDBStorage_Update_withTimeout() {
	ctx := context.Background()
	connStr := "<postgresql connection string>"
...
func ExampleDBStorage_Update_errorContext() {
	connStr := "<postgresql connection string>"

	// Create storage (migrations run automatically in New)
	logger, _ := zap.NewDevelopment()
	storage, err := New(connStr, logger)
	if err != nil {
		fmt.Println("Error creating storage:", err)
	...
	// Use a context that won't timeout
	...
	updated, err := storage.Update(ctx2, metric)
	if err != nil {
		fmt.Println("Error:", err)
		return
	}
